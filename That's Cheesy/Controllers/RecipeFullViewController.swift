...
class RecipeFullViewController: UIViewController {
    ...
    @objc func saveButtonAction(sender: UIButton!) {
        if savedBool == false {
            RecipeManager.shared.addSaved(recipe: Recipe(recipeName: titleLabel.text ?? "", image: imageURL, ingredients: ingredients.text ?? "", instructions: instructions.text ?? "", notes: notes.text ?? "", saved: true))
            saveBtn.setTitle("Unsave", for: .normal)
        } else {
            RecipeManager.shared.removeSaved(recipe: Recipe(recipeName: titleLabel.text ?? "", image: imageURL, ingredients: ingredients.text ?? "", instructions: instructions.text ?? "", notes: notes.text ?? "", saved: false))
